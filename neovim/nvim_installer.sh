...
sh -c 'curl -fLo "${XDG_DATA_HOME:-$HOME/.local/share}"/nvim/site/autoload/plug.vim --create-dirs https://raw.githubusercontent.com/junegunn/vim-plug/master/plug.vim'\
    || die "Could not install vim plugin manager."

# we need to download nvim app image from github.com/neovim/neovim, but we have saved a v0.60 version of it in this repo itself
...
chmod u+x nvim.appimage && ./nvim.appimage || die "Could not execute app image."
...
sudo install lazydocker /usr/local/bin/ || die "Could not install lazydocker unpacked file."
...
echo Neovim successfully set up, now install plugins using :PlugInstall
